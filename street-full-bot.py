...
DRINKS = {
    "cola":   {"name":"🥤 Coca-Cola", "desc":"Klassik Coca-Cola.",
               "sizes":[("0.5 L",8000),("1 L",13000),("1.5 L",16000)]},
    ...
    "fuzetea":{"name":"🍵 Fuze Tea",  "desc":"Choy asosidagi ichimlik.",
               "sizes":[("0.5 L",8000),("1 L",13000),("1.5 L",15000)]},
    "icetea": {"name":"🧊 Ice Tea",   "desc":"Sovuq choy ichimlik.",
               "sizes":[("0.5 L",7000),("1.25 L",13000)]},
    "water":  {"name":"💧 Oddiy suv", "desc":"Toza ichimlik suv.",
               "sizes":[("0.5 L",5000),("1 L",7000),("1.5 L",9000)]},
}

# Barcha mahsulotlar (ichimliklar ham) — savatga qo'shish uchun birlashtirilgan katalog
...
def products_kb():
    ...
    rows.append([InlineKeyboardButton(text="🥤 Ichimliklar", callback_data="cat:drinks")])
...
def drinks_kb():
    rows = [[InlineKeyboardButton(text=d["name"], callback_data=f"p:{did}")] for did, d in DRINKS.items()]
...
def sizes_kb(pid):
    ...
    rows=[[InlineKeyboardButton(text=f"{s} — {money(pr)}",callback_data=f"s:{pid}:{i}")]
          for i,(s,pr) in enumerate(catalog[pid]["sizes"])]
    # Ichimlik bo'lsa orqaga → ichimliklar, aks holda → mahsulotlar
    back_cb = "cat:drinks" if pid in DRINKS else "back:products"
...
def cart_text(cart):
    ...
    for it in cart:
        p=ALL_ALL_PRODUCTS[it["pid"]]; s,pr=p["sizes"][it["si"]]; sub=pr*it["qty"]; total+=sub
        lines.append(f"• {p['name']} ({s}) × {it['qty']} = {money(sub)}")
    lines.append(f"\n💰 <b>Jami: {money(total)}</b>"); return "\n".join(lines)

def cart_total(cart): return sum(ALL_PRODUCTS[i["pid"]]["sizes"][i["si"]][1]*i["qty"] for i in cart)

def items_text(items):
    out=[]
    for it in items:
        p=ALL_ALL_PRODUCTS[it["pid"]]; s,pr=p["sizes"][it["si"]]
...
@router.callback_query(F.data=="cat:drinks")
async def cat_drinks(c):
    await c.message.edit_text("🥤 <b>Ichimliklar</b>\n\nTurini tanlang:", reply_markup=drinks_kb()); await c.answer()

@router.callback_query(F.data.startswith("p:"))
async def choose_product(c):
    pid=c.data.split(":",1)[1]; p=ALL_ALL_PRODUCTS[pid]
...
async def back_sizes(c):
    pid=c.data.split(":")[2]; p=ALL_ALL_PRODUCTS[pid]
    await c.message.edit_text(f"<b>{p['name']}</b>\n<i>{p['desc']}</i>\n\nO'lchamni tanlang:",reply_markup=sizes_kb(pid)); await c.answer()

@router.callback_query(F.data.startswith("s:"))
async def choose_size(c):
    _,pid,si=c.data.split(":"); si=int(si); s,pr=ALL_ALL_PRODUCTS[pid]["sizes"][si]
    await c.message.edit_text(f"<b>{ALL_ALL_PRODUCTS[pid]['name']}</b> — {s}\nNarxi: {money(pr)}\n\nSonini tanlang:",reply_markup=qty_kb(pid,si,1)); await c.answer()

@router.callback_query(F.data.startswith("q:"))
async def change_qty(c):
    _,pid,si,qty=c.data.split(":"); si,qty=int(si),max(1,min(50,int(qty))); s,pr=ALL_ALL_PRODUCTS[pid]["sizes"][si]
...
def _discount_pct_kb(pid, si):
    ...
    p = ALL_PRODUCTS[pid]; sname, orig_price = p["sizes"][si]
    rows = []
...
def _combo_price_kb(items: list):
    ...
    total = sum(ALL_PRODUCTS[it["pid"]]["sizes"][it["si"]][1] for it in items)
    rows = []
...
async def combo_add_item(c: CallbackQuery, state: FSMContext):
    ...
    # Ko'rinish yangilash
    selected_text = "\n".join(
        f"• {ALL_ALL_PRODUCTS[it['pid']]['name']} · {ALL_ALL_PRODUCTS[it['pid']]['sizes'][it['si']][0]} ({money(ALL_ALL_PRODUCTS[it['pid']]['sizes'][it['si']][1])})"
        for it in items)
    total = sum(ALL_PRODUCTS[it["pid"]]["sizes"][it["si"]][1] for it in items)
    await c.message.edit_text(
...
async def combo_done(c: CallbackQuery, state: FSMContext):
    ...
    total = sum(ALL_PRODUCTS[it["pid"]]["sizes"][it["si"]][1] for it in items)
    selected_text = "\n".join(
        f"• {ALL_ALL_PRODUCTS[it['pid']]['name']} · {ALL_ALL_PRODUCTS[it['pid']]['sizes'][it['si']][0]}"
        for it in items)
    await c.message.edit_text(
        f"🎁 <b>Combo narxi</b>\n\n"
        f"Tarkibi:\n{selected_text}\n\n"
...
async def combo_price_chosen(c: CallbackQuery, state: FSMContext):
    ...
    d = await state.get_data()
    items = d.get("combo_items", [])
    orig_total = sum(ALL_PRODUCTS[it["pid"]]["sizes"][it["si"]][1] for it in items)
    items_label = " + ".join(
        f"{ALL_ALL_PRODUCTS[it['pid']]['name']} ({ALL_ALL_PRODUCTS[it['pid']]['sizes'][it['si']][0]})"
        for it in items)
