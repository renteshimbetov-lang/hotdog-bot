...
def cart_text(cart):
    ...
    for it in cart:
        p=ALL_PRODUCTS[it["pid"]]; s,pr=p["sizes"][it["si"]]; sub=pr*it["qty"]; total+=sub
...
def items_text(items):
    out=[]
    for it in items:
        p=ALL_PRODUCTS[it["pid"]]; s,pr=p["sizes"][it["si"]]
...
async def choose_product(c):
    pid=c.data.split(":",1)[1]; p=ALL_PRODUCTS[pid]
...
async def back_sizes(c):
    pid=c.data.split(":")[2]; p=ALL_PRODUCTS[pid]
    await c.message.edit_text(f"<b>{p['name']}</b>\n<i>{p['desc']}</i>\n\nO'lchamni tanlang:",reply_markup=sizes_kb(pid)); await c.answer()

@router.callback_query(F.data.startswith("s:"))
async def choose_size(c):
    _,pid,si=c.data.split(":"); si=int(si); s,pr=ALL_PRODUCTS[pid]["sizes"][si]
    await c.message.edit_text(f"<b>{ALL_PRODUCTS[pid]['name']}</b> — {s}\nNarxi: {money(pr)}\n\nSonini tanlang:",reply_markup=qty_kb(pid,si,1)); await c.answer()

@router.callback_query(F.data.startswith("q:"))
async def change_qty(c):
    _,pid,si,qty=c.data.split(":"); si,qty=int(si),max(1,min(50,int(qty))); s,pr=ALL_PRODUCTS[pid]["sizes"][si]
...
async def combo_add_item(c: CallbackQuery, state: FSMContext):
    ...
    # Ko'rinish yangilash
    selected_text = "\n".join(
        f"• {ALL_PRODUCTS[it['pid']]['name']} · {ALL_PRODUCTS[it['pid']]['sizes'][it['si']][0]} ({money(ALL_PRODUCTS[it['pid']]['sizes'][it['si']][1])})"
        for it in items)
    total = sum(ALL_PRODUCTS[it["pid"]]["sizes"][it["si"]][1] for it in items)
    await c.message.edit_text(
...
async def combo_done(c: CallbackQuery, state: FSMContext):
    ...
    total = sum(ALL_PRODUCTS[it["pid"]]["sizes"][it["si"]][1] for it in items)
    selected_text = "\n".join(
        f"• {ALL_PRODUCTS[it['pid']]['name']} · {ALL_PRODUCTS[it['pid']]['sizes'][it['si']][0]}"
        for it in items)
    await c.message.edit_text(
        f"🎁 <b>Combo narxi</b>\n\n"
        f"Tarkibi:\n{selected_text}\n\n"
