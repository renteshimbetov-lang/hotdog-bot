...
class AdminSt(StatesGroup):
    ...
    promo_type = State(); promo_title = State(); promo_desc = State(); promo_data = State()
...
async def promo_add_start(c: CallbackQuery, state: FSMContext):
    ...
    await state.set_state(AdminSt.promo_title)
    tip = ("🏷 <b>Chegirma aksiyasi</b>" if ptype=="discount" else "🎁 <b>Combo aksiyasi</b>")
    await c.message.answer(f"{tip}\n\nAksiya nomini kiriting (masalan: «Katta Hot-Dog aksiyasi»):")
    await c.answer()

@router.message(AdminSt.promo_title, F.text)
async def promo_title_input(m: Message, state: FSMContext):
    await state.update_data(promo_title=m.text.strip())
    await state.set_state(AdminSt.promo_desc)
    await m.answer("✏️ Qisqa tavsif kiriting (mijozga ko'rinadi):")

@router.message(AdminSt.promo_desc, F.text)
async def promo_desc_input(m: Message, state: FSMContext):
    await state.update_data(promo_desc=m.text.strip())
    d = await state.get_data()
    await state.set_state(AdminSt.promo_data)
    if d["promo_type"] == "discount":
        await m.answer(
            "💸 Narxlarni kiriting quyidagi formatda:\n"
            "<code>mahsulot_nomi|eski_narx|yangi_narx</code>\n\n"
            "Masalan:\n<code>Katta Hot-Dog|25000|20000</code>")
    else:
        await m.answer(
            "🎁 Combo ma'lumotlarini kiriting:\n"
            "<code>tarkibi|combo_narxi</code>\n\n"
            "Masalan:\n<code>Hot-Dog + Kola 0.5L|30000</code>")

@router.message(AdminSt.promo_data, F.text)
async def promo_data_input(m: Message, state: FSMContext):
    d = await state.get_data()
    ptype = d["promo_type"]
    raw = m.text.strip()
    try:
        if ptype == "discount":
            ...
            data_json = json.dumps({"product": parts[0].strip(),
                                    "old_price": int(parts[1].strip()),
                                    "new_price": int(parts[2].strip())}, ensure_ascii=False)
        else:
            parts = raw.split("|")
            if len(parts) != 2: raise ValueError
            data_json = json.dumps({"items": parts[0].strip(),
                                    "price": int(parts[1].strip())}, ensure_ascii=False)
    except Exception:
        await m.answer("⚠️ Format noto'g'ri. Qaytadan kiriting:"); return
    add_promo(ptype, d["promo_title"], d["promo_desc"], data_json)
    await state.clear()
    await m.answer("✅ Aksiya qo'shildi!", reply_markup=main_menu(m.from_user.id))
...
def _cashier_report(rows, label):
    if not rows:
        return f"🧾 <b>Kassiya — {label}</b>\n\nBu davrda to'lov yo'q."
    total = sum(r[3] for r in rows)
    naqd = sum(r[3] for r in rows if "naqd" in (r[4] or "").lower() or "yetkazganda" in (r[4] or "").lower())
    online = total - naqd
    dlv = sum(r[3] for r in rows if r[5]=="delivery")
    pickup = total - dlv
    lines = [
        f"🧾 <b>Kassiya — {label}</b>\n",
        f"💰 Jami tushum: <b>{money(total)}</b>",
        f"  💵 Naqd: {money(naqd)}",
        f"  💳 Online: {money(online)}",
        f"  🚚 Yetkazib berish: {money(dlv)}",
        f"  🏃 Pickup: {money(pickup)}",
        f"\n📋 <b>Batafsil ({len(rows)} ta to'lov):</b>\n",
    ]
    for _, oid, ts, amount, pay_type, otype in rows:
        t = uzt_time(ts, "%d.%m %H:%M")
        oi = "🏃" if otype=="pickup" else "🚚"
        lines.append(f"{oi} #{oid} · {t} · {money(amount)} · {pay_type or '—'}")
    return "\n".join(lines)
...
async def cash_period(c: CallbackQuery):
    ...
    rows = cashier_stats(ts_from, now)
    report = _cashier_report(rows, label)
    kb = InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text="◀️ Orqaga", callback_data="a:cashier")]])
    await c.message.edit_text(report, reply_markup=kb)
    await c.answer()
...
async def cash_confirm_list(c: CallbackQuery):
    if not is_admin(c.from_user.id): await c.answer("Ruxsat yo'q", show_alert=True); return
    # Yetkazilgan lekin kassada tasdiqlanmagan zakazlar
    rows = db.execute(
        "SELECT id,ts,total,pay,order_type FROM orders "
        "WHERE status='delivered' AND id NOT IN (SELECT order_id FROM cashier) "
        "ORDER BY id DESC LIMIT 30").fetchall()
    if not rows:
        await c.answer("✅ Barcha yetkazilgan zakazlar tasdiqlangan!", show_alert=True); return
    kb_rows = []
    for oid, ts, total, pay, otype in rows:
        t = uzt_time(ts, "%d.%m %H:%M")
        oi = "🏃" if otype=="pickup" else "🚚"
        kb_rows.append([InlineKeyboardButton(
            text=f"{oi} #{oid} · {t} · {money(total)}",
            callback_data=f"cash:confirm:{oid}")])
    kb_rows.append([InlineKeyboardButton(text="◀️ Orqaga", callback_data="a:cashier")])
    await c.message.edit_text(
        "💳 <b>Tasdiqlanmagan to'lovlar</b>\n\nZakazni tanlang va to'landi deb belgilang:",
...
async def cash_confirm_order(c: CallbackQuery):
    ...
    _, ts, uid, name, uname, items, total, phone, lat, lon, pay, status, *rest, order_type, _ = row
    ...
    rows = db.execute(
        ...
        "ORDER BY id DESC LIMIT 30").fetchall()
    if not rows:
        await c.message.edit_text("✅ <b>Barcha yetkazilgan zakazlar tasdiqlangan!</b>",
                                   reply_markup=InlineKeyboardMarkup(inline_keyboard=[
                                       [InlineKeyboardButton(text="◀️ Orqaga", callback_data="a:cashier")]]))
        return
    kb_rows = []
    for oid2, ts2, total2, pay2, otype2 in rows:
        t = uzt_time(ts2, "%d.%m %H:%M")
        oi = "🏃" if otype2=="pickup" else "🚚"
        kb_rows.append([InlineKeyboardButton(
            text=f"{oi} #{oid2} · {t} · {money(total2)}",
